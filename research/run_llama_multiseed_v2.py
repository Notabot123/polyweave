"""CORRECTED multi-seed llama-160m (SwiGLU) run (v2). Supersedes run_llama_multiseed.py,
whose numbers were confounded by an under-converged linear baseline on ill-conditioned
...
Run:  venv/Scripts/python.exe -u run_llama_multiseed_v2.py
"""
...
SEEDS = (42, 43, 44)
RAW = Path("plots/raw")
AGG_PATH = RAW / "llama160m_mlp_distill_wikitext2_v2_multiseed.json"
...
def _cfg(seed: int) -> Config:
    return Config(
        ...
        heal_steps=200,
        heal_lr=1e-4,
        results_path=str(RAW / f"llama160m_mlp_distill_wikitext2_v2_seed{seed}.json"),
        plot_prefix=f"polyweave_llama160m_mlp_distill_wikitext2_v2_seed{seed}",
    )
...
def main() -> None:
    for seed in SEEDS:
        path = RAW / f"llama160m_mlp_distill_wikitext2_v2_seed{seed}.json"
        if path.exists():
    ...
    per_seed = [
        json.loads((RAW / f"llama160m_mlp_distill_wikitext2_v2_seed{s}.json").read_text(encoding="utf-8"))
        for s in SEEDS
    ]
