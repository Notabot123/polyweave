...
def run(smoke: bool = False) -> dict:
    ...
    axes[0].axvline(TRAIN_MAX, color="k", linestyle="--", linewidth=1.2, label="train boundary")
    axes[0].set_ylabel("Primality score", fontsize=13)
    axes[0].set_title("DifferentiableSieve (0 params)", fontsize=14)
    axes[0].legend(fontsize=11)
    axes[0].tick_params(labelsize=11)

    axes[1].scatter(x, mlp_scores, s=4, c=true_np, cmap="coolwarm", label="MLP score")
    axes[1].axvline(TRAIN_MAX, color="k", linestyle="--", linewidth=1.2, label="train boundary")
    axes[1].set_ylabel("Sigmoid output", fontsize=13)
    axes[1].set_xlabel("n", fontsize=13)
    axes[1].set_title(f"MLP baseline ({results['mlp_params']} params, trained on [2,{TRAIN_MAX}])", fontsize=14)
    axes[1].legend(fontsize=11)
    axes[1].tick_params(labelsize=11)

    fig.suptitle("Primality detection: structured module vs MLP baseline", fontsize=15)
    fig.tight_layout()
