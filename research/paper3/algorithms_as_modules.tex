...
and 82.5\% out-of-range---never reliably detecting primes, and requiring 7.97\,s of
training and 33{,}409 parameters to do so.
...
the router weight rises monotonically, settling at $w_0 \approx 0.93$ by epoch 400
(range $0.93$--$0.94$ across independent runs).
...
in $(0, 1)$ rather than hard binary labels---the ${\sim}7\%$ residual reflects the MLP
...
\begin{table}[t]
  ...
  \begin{tabular}{lrrrr}
    ...
    MoE (sieve + MLP + router)   & 4{,}483      & 74.8\%           & 82.5\%           & 81.0\%           \\
...
\begin{figure}[t]
  ...
  \caption{Tier-2 MoE results. \emph{Left}: mean router weight toward the sieve expert
    $w_0$ over 400 training epochs, rising from ${\approx}0.58$ to ${\approx}0.93$ without explicit
    supervision. \emph{Centre}: per-$n$ router allocation at inference (red = prime,
    blue = composite); the router assigns high sieve weight uniformly, including in the
    out-of-distribution region $n > 100$. \emph{Right}: accuracy bar chart by range.}
  \label{fig:router}
\end{figure}
