...
Ground truth is computed by \texttt{ForwardChainer} (0 learnable parameters), which is
sound and complete for propositional Horn-clause entailment by construction.
...
chains. \texttt{ForwardChainer} achieves 100\% exact entailment accuracy on both
...
however, soft blending gives 0\% exact accuracy. This is a mathematical certainty
rather than a training failure: polynomial coefficients are large integers (up to
$10{,}240$ in the training set), so any gate weight $w_1 < 1$ scales the exact output
by $w_1 < 1$, shifting every coefficient outside the $\pm 0.5$ integer-rounding
tolerance regardless of training duration or model capacity. No additional training
would recover exactness under soft blending; hard routing is the necessary mechanism. Top-1 routing routes each query to the argmax
...
\begin{figure}[t]
  ...
  \caption{Multi-expert routing (exact experts). \emph{Left}: routing confusion matrix;
    ...
    and binomial queries to \texttt{BinomialExpansion} ($w_1 \approx 0.88$) without
