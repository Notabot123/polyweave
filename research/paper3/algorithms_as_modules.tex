...
\begin{table}[t]
  ...
  \begin{tabular}{llrrrr}
    ...
    Binomial     & MLP baseline                 & 202{,}512  & ---              & ---              & 0.8\%  \\
    \midrule
...
\begin{table}[h]
  ...
  \begin{tabular}{lrrr}
    ...
    Binomial ($A,B \in \{1..4\}$, $n \in \{2..5\}$)   & 0.079 & \textbf{0.910} & 0.012 \\
    \bottomrule
  \end{tabular}
\end{table}
...
for 98\% of binomial queries, exact integer coefficients are recovered in 98.4\% of
training cases and 100\% of OOD cases (binomial OOD queries converge to
$w_1 \rightarrow 1$ more reliably than training queries).
...
\begin{table}[h]
  ...
  \begin{tabular}{lrrrr}
    ...
    \midrule
    Soft mixture  & \textbf{100.0\%} & \textbf{100.0\%} & 0.0\%            & 0.0\%          \\
    Top-1 hard    & \textbf{100.0\%} & \textbf{100.0\%} & 98.4\%           & \textbf{100.0\%} \\
    \bottomrule
  \end{tabular}
\end{table}
...
frozen experts and collapses toward the MLP: gate mass on the MLP rises to 0.616 for
primality and 0.910 for binomial. Primality accuracy falls from 100\% to 74.7\%
...
\begin{figure}[t]
  ...
  \caption{Multi-expert routing (exact experts). \emph{Left}: routing confusion matrix;
    ...
    and binomial queries to \texttt{BinomialExpansion} ($w_1 \approx 0.91$) without
...
for binomial queries ($w_1 = 0.910$) because the joint training loss balances both
...
polynomial queries to the binomial module, achieving near-perfect exact accuracy under
hard routing (100\% primality across all ranges; 98.4\% binomial in-distribution,
100\% binomial OOD).
